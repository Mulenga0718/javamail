...
public class SMTP {
	...
	public static void sendMail(String recepient) {
		Info info = new Info();
		/*
		 * Property에 SMTP서버의 정보를 설정함 ※사용하는 이메일 계정에서 SMTP 사용에 대한 허용도 꼭 해줘야함 Properties :
		 * Properties 클래스는 Hashtables의 하위 클래스 [Key,Value] HashMap과 큰 차이가 없지만, Properties
		 * 클래스는 파일 입출력을 지원한다
		 * 
		 * 
		...
		properties.put("mail.smtp.host", "smtp.naver"
				+ ".com"); // SMTP의 주소 
		properties.put("mail.smtp.port", "587"); // TLS 통신을 할 포트 [네이버 : 587 | 구글 : 465 ]

		final String myAccountEmail = "wjdrbq@naver.com";// 발신자의 메일계정
		final String password = info.pw; // 발신자의 메일 비밀번호
	...
	private static Message prepareMessage(Session session, String myAccountEmail, String recepient) {
		...
		try {
			...
			message.setSubject("My first Email From Java App"); // 메일 제목지정
			message.setText("Hey There, \n Look My Email"); // 메일 내용지정
